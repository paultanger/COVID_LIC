...
```{r fig.align='center', fig.height=6, echo = FALSE, warning = FALSE}
  country = "{{names(combined.plots2[i])}}"
  region = "{{names(combined.plots2[[i]][['map_plot']][j])}}"
  #print(region)
  # try separate.. need to align them
  cases = combined.plots2[[country]]$cases
  crop_plot = combined.plots2[[country]][['crop_plots']][[region]][[1]]
# adjust legend
...
country = "{{names(combined.plots2[i])}}"
region = "{{names(combined.plots2[[i]][['map_plot']][j])}}"
grid.arrange(combined.plots2[[country]][['map_plot']][[region]])
```
:::
::::::
