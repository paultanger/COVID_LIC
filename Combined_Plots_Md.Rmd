...
map.plots = readRDS(paste0(datadir, "map.plots_20200610_1641.RDS"))
```
\newpage
# Cover Page
## Introduction:
...
# can't figure out how to get names as section titles.. pandoc is complicated
...
create_section <- function(i) {
   ...
   # e.g. a random plot
   #plot(sample(1000, 10))
  # get the item in the country list to plot
  #print(grid.draw(map.plots[[names(map.plots[i])]][['map_plot']]))
  #grid.arrange(all.plots[[names(map.plots[i])]][['map_plot']])
...
```{r peak_plots, results='asis', echo=FALSE}
...
for(i in seq(map.plots)){
  #print(names(map.plots[i]))
  #out = c(out, knit_expand('template.rmd')) # this is for child template option
